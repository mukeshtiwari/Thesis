...
$r \in \mathbb{F}$ (the field of integers) is $g^r \cdot h^m$ ($\cdot$ is the group operation).  In Coq, it can be encoded as follows (simplified for presentation):  
...
\noindent
The exponentiation function (NOT RENDERING)  is the scalar multiplication $\cdot$ and the 
function $dot$ is the vector addition $+$ of a vector space of a cyclic group $G$ of prime 
order over the field of integers $\mathbb{F}$ modulo the same prime order.  
...
Equations prod  {F G : Type} {n : nat} 
	(H1 : Group G) (H2 : Field F)
	(H3 : Vector_Space F G)  (^ : G -> F -> G) 
	(dot : G -> G -> G) (hi : Vector.t G (S n)) 
	(mi : Vector.t F (S n)) :  G :=
	prod (^) dot (vcons h vnil) (vcons m vnil) := h ^ m;
	prod (^) dot (vcons h hs)  (vcons m ms)  := 
         dot (h ^ m) (prod (^) dot hs ms).
