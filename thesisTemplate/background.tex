...
\section{Electronic Voting}
  ...
  concern against it. Furthermore, it was already shown in \citep{Wolchok:2010:SAI:1866307.1866309}  that it 
...
%   
%   \begin{enumerate}
%   \item Trusting too much on computers for carrying out the election smoothly 
%    without any mechanism for 
%	\item The software used in election were treated as black-box, lacked   
%	         public scrutiny, and buggy. 
%	 \item The process had no mechanism to incorporate public 
...
%   \end{enumerate}
   ...
   interests of companies \citep{AEC:2013:LMM}. Overall, the lack of transparency 
   in the process of developing software programs 
   
%   
%   The software are developed via a process called software engineering process 
...
%\end{tikzpicture}
 ...
 \noindent	
  The same certificate (augmented with rules and variables instantiated) from checkers'  perspective.
   ...
   If the verified checker returns true, i.e. validates the result, on the certificate generated by unverified 
   vote counting program, then everything is fine from every stakeholders perspective, 
   but what about the situation when the verified checker returns false, i.e. invalidates 
   ...
   specification), 
   a confidence building measure for electoral commission, and the verified checker always returns 
   true on the evidence (certificate) produced by verified vote counting program, confidence booster for 
