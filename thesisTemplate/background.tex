...
%   	 
   ...
   The idea of requiring that computations provide not only results, but also a witness attesting
	...
	and has been put forward in \citep{89397}, \cite{Blum:1995:DPC:200836.200880},
...
\end{tikzpicture}
   ...
   Let us take a simple example of piece of Haskell code, but choice of  language does not matter as long as it 
   produces a certificate, which computes the greatest common divisor 
   of two numbers with a certificate.
   \begin{verbatim}
   ...
   After running the program on Below is the piece of certificate generated by 
   \begin{verbatim}
     gcd 144 89
     -------------
     gcd 89 55
   ...
   In order to verify the correctness of computation of gcd, all we need to do is check there two rules:
   1. \forall x, $\gcd x 0$ = x
   2. \forall x y, gcd x y = gcd y (mod x y)
 
  
  
   
 
  Todo : Move it to the software independence section
  There are several aspects of checker, but some important ones are i) It should be correct, 
  ii) It should be simple,  and iii) It should not take more time than original computation. To achieve 
  correctness, we should formally verify the implementation of checker
