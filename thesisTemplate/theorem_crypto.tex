...
\textbf{Chapter overview:}
 ...
 In the Section \ref{sec:cryptography}, we give some historical 
...
these all variant, including calculus of construction, can be abstracted into one 
...
All these variations of the lambda calculus can be captured into a unified framework
known as \textit{Pure Type System}.  Unlike the simple typed lambda calculus ($\lambda^{\to}$) where 
...
i) set of sorts $S$, ii) set of axioms $A$, and iii) set of rules $R$ such that: 
 ...
 \begin{itemize}
	...
	\item $A$ is the set of axioms of form \textit{c : s} where $c \in C$ and $s \in S$, i.e.  $A \subseteq C \times S$. 
...
appearing at the type level. These assertions at the type level 
...
\label{sec:typeprop}
 ...
 case reification and explain next). 
  ...
  of $A$ is captured by the list $l$). The program is written 
...
First, we defined an abstract syntax tree, \textit{format}, to make explicit the characters we 
...
\label{sec:cryptography}
 ...
 A encryption scheme is homomorphic if for any two plaintext $x$ and $y$:
	    ...
	    Given a public infrastructure ($G, p, g, h$) for ElGamal scheme, 
	     ...
	     If we multiply these two cipher together pairwise, we get ($g^{r_{1}+ r_{2}}, m_{1} * m_{2} *  h^{r_{1} + r_{2}}$). 
     ...
     \label{sec:zkp}
      ...
      the system. For example, we want to prove that  for any group G with group operation *, for any two elements $x \text{  }y \text{  } \in \text{  }G$, we have:
      \begin{displayquote}
      ...
      \noindent
      Proof: we assume arbitrary $x,  \text{  }y$.  We show that $(x * y)}$ and $y^{-1} * x^{-1}$ are 
...
\end{align}
    ...
    Sometimes, the fact that $ x \in L$ is completely trivial \citep{10.1007/3-540-48071-4_28}.  For example, for any given finite group $G$ of order $p$ ($p$ is prime), 
    a random element $h$ from the group $G$, and generator $g$ of the group $G$, a prover claims that there is a $x$ such that 
    $g^x = h$. 
    This is trivial because we know that there always exists such $x$ (because $h \text{  }\in \text{  } G$) ; however, the challenge is to show that
    ...
    the prover $P$.  Moreover, the goal of prover $P$ is to convince verifier $V$ that $(x, w) \in R$ in zero-knowledge, i.e. without revealing
    anything else other than showing that the statement $(x, w) \in R$ is true. 
    
        
   
    \subsection{Sigma Protocol}
    \label{sec:sigma}
     Sigma protocols  \citep{10.1007/3-540-48658-5_19} are efficient way to achieve zero-knowledge proof of knowledge.   Sigma protocol is 
     ...
     \textbf{Completeness:} when prover and verifier follow the protocol for public input $x$ and (private) witness $w$, 
     ...
     \label{sec:commscheme}
      ...
      from protocol. For example, in order to save some time before a match, Indian cricket team captain (coin tossing captain), living in Delhi, and Australian cricket 
      team captain (calling captain), living in Canberra, decide 
      ...
      chat box are in plaintext.   Using the toss-app,  both captains, the coin tossing captain and the calling captain,  post their 
	 ...
	 protocol (coin toss).      
      ...
      The answer is yes, we can devise such scheme.  We would use the sealed lock-box concept, albeit digital one. Moreover, 
      ...
      and post the outcome in a separate digital sealed lock-box  in the chat box.  Now that we two digital sealed lock-box which can 
      only be opened by the respective owners, they would move for the next phase of coin tossing  called reveal phase. 
...
\section{Summary}
In this chapter, we gave a brief summary of Coq theorem prover and cryptographic primitives needed to understand the further chapters. By no means, 
