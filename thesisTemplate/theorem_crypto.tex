...
\section{Cryptography}
   ...
   \begin{itemize}
    ...
    \subsection{Homomorphic Encryption}
     ...
     If a verifier wants to verify the correctness our proof, then he would simply check that if the rules are applied correctly. Moreover, these proofs 
     are static in nature, i.e. once the prover has produced the proof, then the content of proof is not going to change over time. Furthermore,
     there would not be any interaction between prover and verifier if verifier wants to verify the proof.  Also, the verifier 
     ...
     the prover and verifier. Moreover, 
    ...
    a random element h from the group G, and generator of the group g, a prover claims that there is a x such that $g^x = h$. 
    This is trivial because we know that there always exists a x; however, the challenge is to show that the prover knows the witness for 
    this fact. Formally, zero-knowledge-proof of knowledge is defines as: let $R = {(x, w) \subset L x W$ is a binary relation such that     
...
%    An interactive protocol (P, V) is a proof of knowledge for relation R (generally NP-relation) if the
%    \textbf{Completeness:} V accepts the proof only when R x w holds 
%    \textbf{Special Soundness:} There exists a efficient program, also known as extractor, 
%     which can extract the witness if Prover has provided two successful proof transcript. 
%          
    ...
    \subsection{Sigma Protocol}
     ...
     \subsection{Commitment Schemes}   
      Commitment schemes are cryptographic primitives equivalent to real life sealed lock-box which is once locked and sealed, the 
      content inside the lock-box can not be changed.  Basically, these primitives 
      are backbone of any protocol between two parties, communicating over internet, to force them  to  follow the 
      protocol honestly, even if they would have a huge gain from deviating 
      from protocol. For example, for some weird reason, Indian cricket team captain, living in Delhi, and Australian cricket team captain,
      living in Canberra, decide 
      to toss a coin in advance over phone for a  upcoming series of one-day matches to save some time in ground.
      \footnote{
      In cricket, both captains meet in the ground and toss a coin to decide who would have the first call.} Assuming that Australian
      captain is going to toss the coin and Indian captain would  call; if Indian captain says "head" over phone and Australian captain 
      tosses the coin, what would happen if the outcome is "head". Well, depends on how badly Australian wants to have the first call. 
      If he really wants to have the first call, then he would reply on phone: sorry mate, it is tail and you lost.
      \footnote{Story influenced by Manuel Blum's coin flipping by telephone}.  However, we can modify the protocol to force 
      both parties to follow the protocol. Indian captain would keep his call inside a digital lock-box, 
      lock his call inside a sealed digital lock-box and give the box to 
      Australian captain. 
