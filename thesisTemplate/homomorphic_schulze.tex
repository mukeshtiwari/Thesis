...
but encrypting the choices, i.e. the ballot would have been represented as $b: C \to EncryptedNat$
where $EncryptedNat$ is the encrypted representation of a choice (natural number).
...
$bm: C \times C \to \mathbb{N}$,
$bm(x, y)$ where $bm(x, y) = +1$ if $x$ is preferred
...
representation, $bm: C \times C \to \mathbb{N}$,  is that the computation of
the margin matrix is simple, that is
\[ m(x, y) = \sum_{bm \in B} bm(x, y) \]
where $B$ is the multi-set of ballots (in matrix form). 
The major benefit of this
matrix representation is that it can  be transferred to the encrypted setting in a straight
forward way by encrypting all the entries in the matrix $bm$.  We take the advantage of 
matrix representation and represent our encrypted ballot as $encb: C \times C \to \mathbb{Z}$. 
...
encrypted ballots.  In an idea world where there is no cheating, 
...
If we think carefully,  we definitely do not want to be 
...
is ranking all the candidates equal appearing a cycle; however, we 
decided to make it invalid (lets call it invalid undesirable ballot). 
Now during the counting, we need to distinguish between a  desirable 
ballot, i.e.  valid ballot, and an undesirable ballot, i.e. invalid ballot.   
To make this distinction,  we verify that an encrypted matrix ballot $encb$ is valid only if:
\begin{itemize}
