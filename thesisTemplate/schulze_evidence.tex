...
\begin{figure}[h!]
 ...
 and its formal specification in the Section \ref{sec:spec}. The  corner stone of our formalisation is 
...
Pow(C \times C) \to Pow(C \times C)$ (where $Pow$ stand for powerset) , defined by
...
\item the set provided is indeed coclosed
\end{itemize}
...
the protocol.  A complete execution can then be understood as a
...
%the non-existence of paths by exhibiting coclosed sets.
...
constant i.e. 4 (The reason we kept it to 4 candidate to show the speed up compared to 
 \ref{fig:straightslowu} and \ref{fig:straightslown}). During the experiment, we ran an election with 21 candidates, and 
