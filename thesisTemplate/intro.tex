...
\section{Problem Statement}
   ...
   the time saved in was 11,000 working days. 
 ...
 processor. https://github.com/xoreaxeaxeax/sandsifter}, 
   ...
   \item verification, i.e. treated as a black-box which  violates the fundamental property of
   public examinability.
   \end{enumerate}
   
	\noindent
	These untrusted components exposes a large attack surface
	which  could potentially be exploited for illegal gain in election, possibly by 
	current government or foreign country. As a result, a man or women who have 
	not legitimately received a majority or quota would be  put in charge of public office. 
	In a  nutshell, 
    electronic voting violates  fundamental principals of any democratic election, i.e.
    correctness, privacy, and verifiability.
 	

\section{Research Motivation}
This thesis sets out to answer the question of correctness, privacy and verifiability in 
electronic voting. The questions we asked ourself was:
 \begin{enumerate} 
  \item Can we implement a vote counting protocol with the  
    guaranteed correctness of its properties, and practical enough
    to count the real life election involving millions of ballots ? (Correctness and Usability)
...
We achieve the correctness by using formal verification, privacy using 
encryption, and verifiability using scrutiny sheet. We achieve the:
\begin{enumerate}
\item correctness by formally specifying the protocol using the 
 mathematical logic.
 \item verifiability by generating a proof certificate which can later be used to 
 attest the outcome.
 \item privacy by encrypting the ballots.  
\end{enumerate}

 Our approach to achieve the 
correctness is to formally specify the protocol inside theorem prover, and prove its 
